...
const TYPE_ICONS = { request: '📩', status: '📋', feedback: '⭐', system: '🔔' }
    ...
    <div className="min-h-screen bg-dark-900">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          {unread > 0 && (
            <button onClick={handleMarkRead} className="text-sm text-blue-400 hover:text-blue-300">
        ...
        {loading ? <LoadingSpinner /> : notifications.length === 0 ? (
          ...
          <div className="space-y-3">
            {notifications.map(n => (
              <div key={n._id} className={`card ${!n.isRead ? 'border-blue-500/30 bg-blue-500/5' : ''}`}>
                <div className="flex items-start gap-3">
                  ...
                  {!n.isRead && <div className="w-2.5 h-2.5 bg-blue-500 rounded-full shrink-0 mt-1" />}
                </div>
              </div>
            ))}
          </div>
